...
$totalRecords =  mysqli_query($conn, "SELECT COUNT(*) as counts, activity FROM participants where activity != '' AND activity != 'ACTIVITY' AND organisation_type = '$org' group by activity" ) ;
...
$sql = "SELECT COUNT(*) as counts, activity, activity_type, completion_date, activity_type, lead_unit, completion_year  FROM participants where activity != ''  AND activity != 'ACTIVITY' AND organisation_type = '$org' group by activity LIMIT 5 OFFSET " . ($page - 1) * 5 ;
...
$sqltab = "SELECT * FROM participants where activity = '".$rowxx["activity"]."' AND organisation_type = '$org'";
